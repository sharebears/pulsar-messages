...
def view_conversations(user: User,
    ...
    return flask.jsonify({
        'conversations_count': PrivateConversation.count_from_user(user.id, filter=filter),
...
def create_conversation(topic: str,
                        ...
                        message: str):
    if len(recipient_ids) > 1 and not flask.g.user.has_permission(MessagePermissions.MULTI_USER):
