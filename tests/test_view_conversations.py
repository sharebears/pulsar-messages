...
def test_view_conversations_paginated_page(app, authed_client):
    response = authed_client.get(
        '/messages/conversations', query_string={'page': 2}
    ).get_json()
    response = response['response']
...
def test_view_sentbox(app, authed_client):
    response = authed_client.get(
        '/messages/conversations', query_string={'filter': 'sentbox'}
    ).get_json()
    response = response['response']
...
def test_view_conversations_others(app, authed_client):
    ...
    response = authed_client.get(
        '/messages/conversations', query_string={'user_id': 2}
    ).get_json()
    response = response['response']
    assert len(response['conversations']) == 3
    assert all(c['id'] in {1, 2, 3} for c in response['conversations'])
...
def test_view_conversation(app, authed_client):
    assert (
        PrivateConversationState.from_attrs(conv_id=1, user_id=1).read is False
    )
...
def test_view_conversation_pagination(app, authed_client):
    response = authed_client.get(
        '/messages/conversations/1', query_string={'page': 2}
    )
    json = response.get_json()['response']
    ...
    assert (
        PrivateConversationState.from_attrs(conv_id=1, user_id=1).read is True
    )
...
def test_view_conversation_deleted(app, authed_client):
    ...
    db.session.commit()
    response = authed_client.get('/messages/conversations/1').get_json()[
        'response'
    ]
...
def test_delete_conversation(app, authed_client):
    ...
    response = authed_client.put(
        '/messages/conversations/1', data=json.dumps({'deleted': True})
    ).get_json()['response']
...
def test_mark_conversation_as_read(app, authed_client):
    add_permissions(app, MessagePermissions.VIEW_DELETED)
    response = authed_client.put(
        '/messages/conversations/1', data=json.dumps({'read': True})
    ).get_json()['response']
    assert response == 'Successfully modified conversation 1.'
...
def test_modify_conversation_not_member_of(app, authed_client):
    response = authed_client.put('/messages/conversations/4').get_json()[
        'response'
    ]
    assert (
        response
...
def test_delete_conversation_permission_override(app, authed_client):
    ...
    assert (
        PrivateConversationState.from_attrs(conv_id=4, user_id=2).deleted
        is True
    )
...
def test_modify_conversation_in_bulk(app, authed_client):
    response = authed_client.put(
        '/messages/conversations',
        data=json.dumps({'conversation_ids': [1, 3], 'read': True}),
    ).get_json()['response']
    ...
    assert (
        PrivateConversationState.from_attrs(conv_id=3, user_id=1).read is True
    )
...
def test_modify_conversation_in_bulk_no_perms(app, authed_client):
    response = authed_client.put(
        '/messages/conversations',
        data=json.dumps({'conversation_ids': [1, 2, 4], 'read': True}),
    ).get_json()['response']
    assert (
        response
        == 'You cannot modify conversations that you are not a member of: 4.'
    )
    assert (
        PrivateConversationState.from_attrs(conv_id=1, user_id=1).read is False
    )
...
def test_delete_conversation_in_bulk(app, authed_client):
    response = authed_client.put(
        '/messages/conversations',
        data=json.dumps({'conversation_ids': [1, 2], 'deleted': True}),
    ).get_json()['response']
    assert response == 'Successfully modified conversations 1, 2.'
    assert (
        PrivateConversationState.from_attrs(conv_id=1, user_id=1).deleted
        is True
    )
    assert (
        PrivateConversationState.from_attrs(conv_id=2, user_id=1).deleted
        is True
    )
