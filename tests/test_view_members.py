...
def test_add_members_to_conversation_already_in(app, authed_client):
    ...
    assert response == 'The following members are already in the conversation: user_two.'
...
def test_delete_nonexistent_members_from_conversation(app, authed_client):
    ...
    assert response == \
        'The following user_ids are not in the conversation: 5.'


@pytest.mark.parametrize(
    'endpoint, method', [
        ('/messages/1/members', 'POST'),
    ])
